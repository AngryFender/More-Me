...
with ui.card().classes('w-full'):  # Set width to full screen
    ...
    with ui.grid().classes("w-full").style("grid-template-columns:  50px auto 60px"):
        ...
        lb_bb= ui.label('1')
        ui.label('Vocal')
        ui.slider(min=1, max=100).bind_value(demo, 'bass_vocal').on_value_change(lambda: send_midi_serial('bassist',61, demo.bass_bass, lb_bv))
        lb_bv= ui.label('1')
        ui.label('Prs')
        ui.slider(min=1, max=100).bind_value(demo, 'bass_prs').on_value_change(lambda: send_midi_serial('bassist',62, demo.bass_bass, lb_bp))
        lb_bp= ui.label('1')
        ui.label('Tele')
        ui.slider(min=1, max=100).bind_value(demo, 'bass_tele').on_value_change(lambda: send_midi_serial('bassist',63, demo.bass_bass, lb_bt))
        lb_bt= ui.label('1')
        ui.label('Drums')
        ui.slider(min=1, max=100).bind_value(demo, 'bass_drums').on_value_change(lambda: send_midi_serial('bassist',64, demo.bass_bass, lb_bd))
        lb_bd= ui.label('1')
        ui.label('Volume')
        ui.slider(min=1, max=100).bind_value(demo, 'bass_volume').on_value_change(lambda: send_midi_serial('bassist',65, demo.bass_bass, lb_bov))
        lb_bov= ui.label('1')
